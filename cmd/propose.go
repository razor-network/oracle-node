...
func getIteration(client *ethclient.Client, address string, proposer types.ElectedProposer, proposeUtils proposeUtilsInterface, razorUtils utilsInterface) int {
	for i := 0; i < 10000000000; i++ {
		proposer.Iteration = i
		isElected := proposeUtils.isElectedProposer(client, address, proposer, razorUtils)
		if isElected {
			return i
		}
	}
...
func isElectedProposer(client *ethclient.Client, address string, proposer types.ElectedProposer, razorUtils utilsInterface) bool {
	...
	influence, err := razorUtils.GetInfluenceSnapshot(client, address, proposer.StakerId, proposer.Epoch)
	if err != nil {
