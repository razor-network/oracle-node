...
func (utilsStruct UtilsStruct) SetDelegation(flagSet *pflag.FlagSet) error {
	...
	if commission != 0 {
		...
		if stakerInfo.Commission == 0 {
			...
			setCommissionTxnOpts := utilsStruct.razorUtils.GetTxnOpts(txnOpts, utilsStruct)
		...
		if stakerInfo.Commission > 0 && stakerInfo.Commission > commission {
			...
			txnOpts.Parameters = []interface{}{commission}
			decreaseCommissionTxnOpts := utilsStruct.razorUtils.GetTxnOpts(txnOpts, utilsStruct)
	...
	if stakerInfo.AcceptDelegation != status {
		...
		setDelegationAcceptanceTxnOpts := utilsStruct.razorUtils.GetTxnOpts(txnOpts, utilsStruct)
